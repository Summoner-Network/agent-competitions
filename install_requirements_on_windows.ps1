...
# Collect requirement files: agent_InputAgent\requirements.txt and agent_templates\*\requirements.txt
$files = @()


if (Test-Path -LiteralPath (Join-Path $PSScriptRoot "agent_templates")) {
  $files += Get-ChildItem -LiteralPath (Join-Path $PSScriptRoot "agent_templates") -Recurse -Filter "requirements.txt" -ErrorAction SilentlyContinue |
            Where-Object { -not $_.PSIsContainer }
}

# agent_InputAgent\requirements.txt
$inputReq = Join-Path $PSScriptRoot "agent_InputAgent\requirements.txt"
if (Test-Path -LiteralPath $inputReq) {
  $files += Get-Item -LiteralPath $inputReq
}

if (-not $files -or $files.Count -eq 0) {
  Write-Host -ForegroundColor Yellow "No requirements.txt files found under agent_InputAgent\requirements.txt or agent_templates\*\requirements.txt."
  return
}
